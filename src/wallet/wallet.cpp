...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CAmount& nFees, CMutableTransaction& tx, CKey& key)
{
    ...
    // Calculate reward
    {
        int64_t nReward = GetProofOfStakeSubsidy(pindexPrev, nFees);
        // block block fees are included in getprofofstakesubsidy, when we are in running fees if not, add it back
...
CAmount CWalletTx::GetCredit(const isminefilter& filter, const int required_confirms) const
{
    // Must wait until coinbase is safely deep enough in the chain before valuing it
    if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > (120 - required_confirms))
        return 0;
